...
async def stream_status_by_batch(request: Request, channel_type: str, batch_id: str):
    ...
    try:
        ...
        return StreamingResponse(
            iter(
                [
                    'data: {"action": "error", "message": "Could not connect to stream server."}\n\n'
                ]
            ),
    ...
    if channel_type not in ["resume-status", "pool-status"]:
        return StreamingResponse(
            iter(
                [
                    f"data: {json.dumps({'action': 'error', 'message': 'Invalid channel type specified.'})}\n\n"
                ]
            ),
            media_type="text/event-stream",
        )
        ...
        try:
            ...
            while True:
                ...
                if await request.is_disconnected():
                    logger.warning(
                        f"🔌 Client disconnected from channel: {channel}. Closing stream."
                    )
                    break
